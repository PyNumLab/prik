...
- character arrays that cannot be represented as a fixed-width NumPy bytes
  dtype, `allocatable` and `pointer` character *fields*, and assumed-length
  (`character(len=*)`) character module state, whose width comes from an
  initializer prik does not evaluate.
