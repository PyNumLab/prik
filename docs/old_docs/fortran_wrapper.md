...
This guide describes the Python API generated by prik for Fortran code. It is
...
multi-source build, prik merges the public surface into that extension and
...
generate an editable `Makefile.prik` without compiling. These modes are mutually
exclusive.
...
Ownership decisions are centralized in `prik.ownership_policy`. Semantic
...
array function results are returned as new Python-owned NumPy arrays. prik does
...
`.pyi` suppresses that constructor; prik does not regenerate it. To use one
...
```bash
python3 -m prik generate --makefile mesh.f90 solver.f90 --out-dir build --json
...
```bash
python3 -m prik generate --makefile parallel_api.f90 --out-dir build --json
...
General borrowed pointer views are not supported. prik cannot yet:
