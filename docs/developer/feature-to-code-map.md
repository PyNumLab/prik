...
| C inspection and semantic IR | [Parsers](packages/parsers.md), [semantics](packages/semantics.md) | `prik/preprocessing/c.py` → `prik/parsers/c/parser.py` → `prik/semantics/c2ir.py` → `prik/semantics/models.py` | `tests/c/infrastructure/preprocessing/`, `tests/c/infrastructure/parsing/`, `tests/c/infrastructure/semantic_ir/semantics/` |
| CLI commands and reports | [Beginner workflow](../user/getting-started/beginner-workflow.md) | `prik/cli.py` → the selected `prik/parsers/{fortran,c}/cli.py` report path | `tests/fortran/infrastructure/cli/pipeline/`, `tests/c/infrastructure/cli/pipeline/`, `tests/docs/test_examples.py` |
