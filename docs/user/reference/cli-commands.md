...
| `--no-standard-logicals` | Omits the compiler option that gives a Fortran `logical` the representation C expects (`-standard-semantics` on Intel, `-Munixlogical` on PGI/NVIDIA). Use only to link prebuilt Intel objects compiled without it, since that option also changes module symbol mangling. |
