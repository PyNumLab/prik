...
| Scalar kind coverage | Supported | [Data types](../guide/data-types.md) | [Scalar kind tests](../../../tests/fortran/data_types/end_to_end/test_primitive_scalar_runtime.py) | Real and complex storage wider than the target's `long double` is blocked; `real(10)` and C `long double` map to NumPy `longdouble`. All logical kinds are supported: scalar values use Python `bool`, while arrays use matching-width NumPy storage. |
