...
Use a matching C and Fortran compiler from the same vendor/family.
A generated binding can include the Fortran runtime's `ISO_Fortran_binding.h`,
which a C compiler from another vendor does not find: Apple Clang beside
a Homebrew GNU Fortran fails to compile the binding, while GNU `gcc`
beside GNU Fortran resolves it. The toolchain stays CMake's to choose,
so name the pair through the usual `CMAKE_C_COMPILER` and `CMAKE_Fortran_COMPILER`,
or `CMAKE_ARGS` when a build backend drives the configure step.
