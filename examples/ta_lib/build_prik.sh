...
if ! TA_LIB_PREFIX="$(python3 "$EXAMPLE_WORKSPACE/examples/ta_lib/native_build.py" \
  --compiler "$TA_LIB_COMPILER_PATH")"; then
  return 1 2>/dev/null || exit 1
fi
export TA_LIB_PREFIX
if ! TA_LIB_REGTEST_CACHE="$(python3 "$EXAMPLE_WORKSPACE/examples/ta_lib/native_build.py" \
  --compiler "$TA_LIB_COMPILER_PATH" --artifact regtest)"; then
  return 1 2>/dev/null || exit 1
fi
if ! TA_LIB_REFERENCE_CACHE="$(python3 "$EXAMPLE_WORKSPACE/examples/ta_lib/native_build.py" \
  --compiler "$TA_LIB_COMPILER_PATH" --artifact reference)"; then
  return 1 2>/dev/null || exit 1
fi
if ! TA_LIB_SHARED_LIBRARY="$(python3 "$EXAMPLE_WORKSPACE/examples/ta_lib/native_build.py" \
...
if ! python3 -m prik generate --pyi --language c \
  "$EXAMPLE_WORKSPACE/examples/ta_lib/ta_lib_probe.h" \
...
if ! python3 -m prik --language c "$EXAMPLE_WORKSPACE/examples/ta_lib/ta_lib_api.pyi" \
...
cp "$EXAMPLE_WORKSPACE/examples/ta_lib/reference_adapter.py" "$TA_LIB_BUILD_ROOT/reference/ta_codegen_serve_c"
chmod +x \
...
export PRIK_TALIB_CONTRACT="$EXAMPLE_WORKSPACE/examples/ta_lib/ta_lib_api.pyi"
