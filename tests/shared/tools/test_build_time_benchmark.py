...
def test_build_commands_use_the_same_sources_and_complete_optimization_flags(tmp_path: Path) -> None:
    ...
    assert x2py[x2py.index("--jobs") + 1] == "4"
    assert "--native-compile-flags=-O0" in x2py
    assert "--wrapper-fortran-flags=-O0" in x2py
    assert "--wrapper-c-flags=-O0" in x2py
...
def test_run_build_benchmarks_excludes_warmups_and_writes_paired_pyperf_suites(
    ...
    first_round = [(tool, name) for name in case_names for tool in ("x2py", "f2py")]
    second_round = [(tool, name) for name in case_names for tool in ("x2py", "f2py")]
    third_round = [(tool, name) for name in case_names for tool in ("f2py", "x2py")]
    ...
    assert x2py_suite.get_benchmark_names() == case_names
    assert f2py_suite.get_benchmark_names() == case_names
    assert x2py_suite.get_benchmark(case_names[0]).get_nvalue() == 2
