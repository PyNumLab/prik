...
module fbind_value_f90
  use iso_c_binding
contains
  integer(c_int) function plus_value(n) bind(C, name="prik_plus_value") result(res)
  ...
  real(c_double) function scale_real(x) bind(C, name="prik_scale_real") result(res)
    real(c_double), value, intent(in) :: x

    res = 2.5_c_double * x
  end function scale_real

  complex(c_double_complex) function conjugate_value(z) bind(C, name="prik_conjugate_value") result(res)
    complex(c_double_complex), value, intent(in) :: z

    res = conjg(z)
  end function conjugate_value

  logical(c_bool) function invert_flag(flag) bind(C, name="prik_invert_flag") result(res)
