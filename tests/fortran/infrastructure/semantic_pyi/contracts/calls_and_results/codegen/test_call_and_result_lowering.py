...
""",
    ...
    assert reordered.bridge.standalone is True
...
""",
    ...
    assert [
        (slot.source_kind, slot.literal_type, slot.literal_value) for slot in hidden.entrypoint.projected_slots
    ] == [
        ("literal", "Int32", 1),
    ...
    assert hidden.results[0].projected_call_slot.native_position == 3
    assert hidden.results[0].bridge.native_action is NativeBarrierAction.PASS_CALL_LOCAL_ADDRESS
    assert hidden.results[0].projected_call_slot.object_kind is ObjectKind.SCALAR
