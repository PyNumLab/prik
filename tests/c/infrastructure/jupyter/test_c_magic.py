...
def test_handwritten_c_contract_builds_an_existing_source_without_generated_metadata(
    tmp_path: Path,
    monkeypatch,
):
    modules: dict[str, ModuleType] = {}
    ...
    shell = _Shell()
    magic = PrikMagics(shell, cache_dir=tmp_path / "cache")
    cell = "from prik.contracts import Float64\n\ndef square(value: Float64) -> Float64: ...\n"

    magic.pyi(
        f'--native-c-sources {source} --compiler clang --native-compile-flags="-O3 -std=c11"',
        cell,
    )

    contract, kwargs = calls[0]
    assert contract.name == "contract.pyi"
    assert contract.read_text(encoding="utf-8") == cell
    assert kwargs["native_language"] == "c"
    assert kwargs["input_c_compiler"] == "clang"
