...
def test_x2py_parse_c_path_preserves_parser_and_preprocessing_arguments(
    tmp_path: Path,
    monkeypatch,
):
    path = tmp_path / "api.h"
    ...
    compiler_config = x2py_cli.PreprocessingConfig(
        mode="compiler",
        compiler="cc",
...
def test_x2py_main_preserves_c_parse_error_rendering_contract(monkeypatch, capsys):
    args = _main_args(language="c", parse=True, no_color=True)
    _install_main_parser(monkeypatch, args)
    preprocessing = type("Preprocessing", (), {"include_dirs": ()})()
    error = x2py_cli.CParseError("bad parse")
    calls = []
...
def test_x2py_main_reraises_c_parse_errors_for_debug_environment(monkeypatch):
    ...
    with pytest.raises(x2py_cli.CParseError):
        x2py_cli.main()
